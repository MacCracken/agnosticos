# Getting Support

Thank you for using AGNOS! Here's how to get help.

## Documentation

Before opening an issue, please check the documentation:

| Resource | Description |
|----------|-------------|
| [README.md](README.md) | Project overview and quick start |
| [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) | System architecture |
| [docs/AGENT_RUNTIME.md](docs/AGENT_RUNTIME.md) | Agent runtime guide |
| [docs/TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md) | Common issues and solutions |
| [docs/development/roadmap.md](docs/development/roadmap.md) | Development roadmap |
| [docs/api/](docs/api/) | API reference |

## Community Channels

| Channel | Best For |
|---------|---------|
| [GitHub Discussions](https://github.com/agnostos/agnos/discussions) | General questions, ideas, show & tell |
| [Matrix: #agnos:matrix.org](https://matrix.to/#/#agnos:matrix.org) | Real-time chat |
| [Discord](https://discord.gg/agnos) | Community chat and support |
| [Mastodon: @agnos@fosstodon.org](https://fosstodon.org/@agnos) | Announcements |

## Bug Reports & Feature Requests

- **Bug report**: [Open a GitHub Issue](https://github.com/agnostos/agnos/issues/new?template=bug_report.md)
- **Feature request**: [Open a GitHub Issue](https://github.com/agnostos/agnos/issues/new?template=feature_request.md)

Before opening an issue, please:
1. Search [existing issues](https://github.com/agnostos/agnos/issues) to avoid duplicates
2. Check the [troubleshooting guide](docs/TROUBLESHOOTING.md)
3. Gather relevant logs and system information

## Security Vulnerabilities

**Do NOT report security vulnerabilities in public GitHub Issues.**

Please follow the responsible disclosure process described in [SECURITY.md](SECURITY.md).

## Contributing

Want to fix a bug or add a feature yourself? See [CONTRIBUTING.md](CONTRIBUTING.md) to get started.

## Status Page

> During pre-alpha, there is no dedicated status page. Monitor the
> [GitHub repository](https://github.com/agnostos/agnos) for announcements.
