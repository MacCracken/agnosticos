# AGNOS Applications & Libraries

> Documentation for all first-party AGNOS software.
>
> See also: [First-Party Standards](../development/applications/first-party-standards.md) | [Application Roadmap](../development/applications/roadmap.md) | [Monolith Extraction](../development/monolith-extraction.md)

---

## Consumer Applications

Desktop, CLI, and service applications built on the AGNOS stack.

| App | Domain | Docs |
|-----|--------|------|
| [Abacus](abacus.md) | Desktop calculator (GUI) | Built on abaco |
| [Aequi](aequi.md) | Self-employed accounting (Tauri) | |
| [Agnostic](agnostic.md) | Python/CrewAI agent platform | |
| [BullShift](bullshift.md) | Trading platform | |
| [Delta](delta.md) | Code hosting (git, PRs, CI/CD) | Port 8070 |
| [Irfan](irfan.md) | Local LLM management/training | |
| [Jalwa](jalwa.md) | Media player | Built on tarang |
| [Mneme](mneme.md) | Knowledge base | |
| [Muharrir](muharrir.md) | Text editor | |
| [Nazar](nazar.md) | System monitor | Port 8095 |
| [PhotisNadi](photisnadi.md) | Productivity app | |
| [Rahd](rahd.md) | Calendar & contacts | |
| [Rasa](rasa.md) | Image editor | |
| [SecureYeoman](secureyeoman.md) | TypeScript/Bun agent platform | |
| [Selah](selah.md) | Screenshot & annotation | |
| [Shruti](shruti.md) | DAW (digital audio workstation) | |
| [Sutra](sutra.md) | Infrastructure orchestrator | |
| [Tazama](tazama.md) | Video editor | |
| [Vidhana](vidhana.md) | File manager | |

## Shared Library Crates

Reusable libraries that form the stack. See **[libs/](libs/README.md)** for the full categorized index.

## Third-Party Packages

Packaged external software for AGNOS. See **[thirdparty/](thirdparty/)**.
