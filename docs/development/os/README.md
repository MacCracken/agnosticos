# OS Subsystems — In Development (pre-1.0)

OS-level subsystems not yet at v1.0. Each is a standalone repo.

## Core Services

- [daimon](daimon.md) — Agent orchestrator (0.5.0)
- [agnoshi](agnoshi.md) — AI shell (0.1.0)
- [aethersafha](aethersafha.md) — Wayland compositor (0.1.0)

## Trust & Security

- [aegis](aegis.md) — Security daemon (0.1.0)
- [t-ron](t-ron.md) — MCP security monitor (0.90.0)
- [phylax](phylax.md) — Threat detection (0.22.3)

## MCP & Infrastructure

- [bote](bote.md) — MCP core (0.90.0)
- [libro](libro.md) — Audit chain (0.25.3)
- [nein](nein.md) — Firewall (0.24.3)
- [yukti](yukti.md) — Device abstraction (0.25.3)
- [samay](samay.md) — Task scheduler (0.1.0)

## Package Management

- [ark](ark.md) — Package manager (0.1.0)
- [nous](nous.md) — Package resolver (0.1.0)
- [takumi](takumi.md) — Build system (0.1.0)
- [mela](mela.md) — App marketplace (0.1.0)

## Init & Install

- [argonaut](argonaut.md) — Init system (0.1.0)
- [agnova](agnova.md) — OS installer (0.1.0)

## Edge

- [seema](seema.md) — Edge fleet management (0.1.0)

---

v1.0+ OS subsystems: [docs/os/](../../os/README.md)
Non-OS domain libraries: [docs/applications/libs/](../../applications/libs/)
