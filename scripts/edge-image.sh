#!/bin/bash
# edge-image.sh — Build AGNOS Edge OS bootable images
#
# Produces minimal, dm-verity-protected bootable images for edge devices.
# Supports x86_64 (EFI ISO) and aarch64 (RPi SD card .img).
#
# Usage:
#   ./scripts/edge-image.sh x86_64          # Build EFI-bootable ISO
#   ./scripts/edge-image.sh aarch64         # Build RPi SD card image
#   ./scripts/edge-image.sh --help
#
# Requirements:
...
build_x86_64_iso() {
    ...
    local iso_out="$OUTPUT_DIR/agnos-edge-${AGNOS_VERSION}-x86_64.iso"
...
build_aarch64_img() {
    ...
    local img_out="$OUTPUT_DIR/agnos-edge-${AGNOS_VERSION}-aarch64.img"
    ...
    if [[ -d "$BUILD_DIR/boot/dtbs" ]]; then
        cp -r "$BUILD_DIR/boot/dtbs" "$mnt_boot/"
    fi
...
print_summary() {
    ...
    case "$ARCH" in
        x86_64)
            ...
            log_info "  qemu-system-x86_64 -m 512 -cdrom $OUTPUT_DIR/agnos-edge-${AGNOS_VERSION}-x86_64.iso"
            ;;
        aarch64)
            log_info "Flash to SD card and boot on Raspberry Pi 4/5:"
            log_info "  sudo dd if=$OUTPUT_DIR/agnos-edge-${AGNOS_VERSION}-aarch64.img of=/dev/sdX bs=4M conv=fsync"
            ;;
    esac
    echo ""
}
