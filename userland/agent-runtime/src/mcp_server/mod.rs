...
async fn dispatch_tool_call(
    ...
    match call.name.as_str() {
        ...
        "agnos_gpu_probe" => handle_gpu_probe_json(&call.arguments).await,
        "agnos_gpu_recommend" => handle_gpu_recommend(&call.arguments).await,
        ...
        "agnostic_submit_task" => handle_agnostic_submit_task(&call.arguments).await,
        "agnostic_task_status" => handle_agnostic_task_status(&call.arguments).await,
        ...
        "agnostic_dashboard" => handle_agnostic_dashboard(&call.arguments).await,
        "agnostic_list_sessions" => handle_agnostic_list_sessions(&call.arguments).await,
        "agnostic_agent_status" => handle_agnostic_agent_status(&call.arguments).await,
        "agnostic_quality_trends" => handle_agnostic_quality_trends(&call.arguments).await,
        "agnostic_session_diff" => handle_agnostic_session_diff(&call.arguments).await,
        "agnostic_agent_metrics" => handle_agnostic_agent_metrics(&call.arguments).await,
        "agnostic_llm_usage" => handle_agnostic_llm_usage(&call.arguments).await,
        "agnostic_health" => handle_agnostic_health(&call.arguments).await,
        "agnostic_preset_recommend" => handle_agnostic_preset_recommend(&call.arguments).await,
        "agnostic_a2a_delegate" => handle_agnostic_a2a_delegate(&call.arguments).await,
        "agnostic_a2a_status" => handle_agnostic_a2a_status(&call.arguments).await,
        "agnostic_a2a_heartbeat" => handle_agnostic_a2a_heartbeat(&call.arguments).await,
        ...
        "agnostic_crew_gpu" => handle_agnostic_crew_gpu(&call.arguments).await,
