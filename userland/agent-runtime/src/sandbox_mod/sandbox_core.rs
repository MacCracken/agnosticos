...
impl Sandbox {
    ...
    pub async fn apply_with_profile(
        &mut self,
        profile: &crate::sandbox_mod::seccomp::SeccompProfile,
    ) -> Result<()> {
        if self.applied {
            return Ok(());
        }
        ...
        crate::sandbox_mod::seccomp::validate_profile(profile)
            .map_err(|e| anyhow::anyhow!("Invalid seccomp profile: {}", e))?;

        // Build the profile-specific filter spec (for logging/audit)
        let filter_spec = crate::sandbox_mod::seccomp::build_seccomp_filter(profile);
        debug!(
...
mod tests {
    ...
    async fn test_sandbox_apply_with_profile() {
        ...
        let _result = sandbox
            .apply_with_profile(&crate::sandbox_mod::seccomp::SeccompProfile::Python)
            .await;
    }
    ...
    async fn test_sandbox_apply_with_invalid_profile() {
        ...
        let empty_profile = crate::sandbox_mod::seccomp::SeccompProfile::Custom(vec![]);
    ...
    async fn test_sandbox_apply_with_profile_idempotent() {
        ...
        // First apply
        let _result = sandbox
            .apply_with_profile(&crate::sandbox_mod::seccomp::SeccompProfile::Shell)
            .await;
    ...
    async fn test_sandbox_apply_with_all_profiles() {
        let profiles = [
            crate::sandbox_mod::seccomp::SeccompProfile::Python,
            crate::sandbox_mod::seccomp::SeccompProfile::Node,
            crate::sandbox_mod::seccomp::SeccompProfile::Shell,
            crate::sandbox_mod::seccomp::SeccompProfile::Wasm,
        ];
